<template>
	<div class="menu-item-form q-gutter-y-md">
		...
		<q-input
			class="menu-item-form__sub-title"
			type="textarea"
			clearable
			autogrow
			ref="subTitle"
			v-model="menuItem.subTitle"
			:label="$tl('subTitle')"
		...
		>
			...
			<div slot="hint">
				...
				<div v-else-if="urlError" class="text-red-8">[{{ $tl("urlError") }}]</div>
			</div>
		</q-input>

		<q-field
			bottom-slots
			class="menu-item-form__parent cursor-pointer"
			v-if="parentOptions"
			:label="$tl('parent')"
			stack-label
		>
		...
		<q-input
			class="menu-item-form__css-class"
			ref="cssClass"
			clearable
			v-model="menuItem.cssClass"
			:label="$tl('cssClass')"
		...
		<q-input
			clearable
			class="menu-item-form__settings-json"
			ref="settingsJson"
			type="textarea"
			v-model="menuItem.settingsJson"
			autogrow
		...
		<q-checkbox
			class="menu-item-form__exact"
			ref="exact"
			v-model="menuItem.exact"
			:label="$tl('exact')"
		/>
		...
		<q-checkbox
			class="menu-item-form__is-hidden"
			ref="isHidden"
			v-model="menuItem.isHidden"
			:label="$tl('isHidden')"
		/>
	</div>
</template>
...
function createRules() {
	return {
		...
		title: [
			...
			value =>
				value.length <= config.DbColumnSizes.MenuItems_Title ||
				this.$tl("validation.title.maxLength")
		],
		subTitle: [
			value =>
				!value || value.length >= 3 || this.$tl("validation.subTitle.minLength"),
			value =>
				(!value || value.length) <= config.DbColumnSizes.MenuItems_SubTitle ||
				this.$tl("validation.subTitle.maxLength")
		],
		url: [],
		cssClass: [
			value =>
				!value || value.length >= 3 || this.$tl("validation.cssClass.minLength"),
			value =>
				(!value || value.length) <= config.DbColumnSizes.MenuItems_SubTitle ||
				this.$tl("validation.cssClass.maxLength")
		],
		icon: [
			value =>
...
export default {
	...
	computed: {
		parentTitle() {
			...
			if (!this.menuItemsById[key]) return "";
			else return this.menuItemsById[key].title;
		},
		parentIcon() {
			const key = this.menuItem.parentId;
			if (!this.menuItemsById[key]) return null;
			else return this.menuItemsById[key].icon;
		},
		hasError() {
			return (
			...
			if (this.menuItem.routeName) {
				const resolved = this.$router.resolve({
					...
					params: this.menuItem.routeParamsJson
						? JSON.parse(this.menuItem.routeParamsJson)
						: undefined
				});
		...
		prepareMenuItems(allMenuItems) {
			...
			for (let menuItem of allMenuItems) {
				if (menuItem.parentId) {
					...
					if (!menuItem.icon) menuItem.icon = "far fa-file";
				} else {
					root = menuItem;
