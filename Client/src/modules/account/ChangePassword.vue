            ...
            <div class="center-form">
                ...
                <q-input class="change-password__password-old" ref="passwordOld" v-model="passwordOld"
                    ...
                    <template v-slot:append>
                        <q-icon :name="showPasswordOld ? 'far fa-eye' : 'far fa-eye-slash'" class="cursor-pointer"
                                @click="showPasswordOld = !showPasswordOld"/>
                    </template>
                </q-input>

                <q-input class="change-password__password1" ref="password" v-model="password"
                         :type="showPassword ? 'text' : 'password'" :label="$tl('password')" :rules="rules.password">
                    <template v-slot:prepend>
                        <q-icon name="fas fa-key"/>
                    </template>
                    <template v-slot:append>
                ...
                <q-input class="change-password__password2" ref="password2" v-model="password2"
                         :type="showPassword2 ? 'text' : 'password'" :label="$tl('password2')" :rules="rules.password2">
                    <template v-slot:prepend>
                        <q-icon name="fas fa-key"/>
                    </template>
                    <template v-slot:append>
                        <q-icon :name="showPassword2 ?  'far fa-eye' : 'far fa-eye-slash'" class="cursor-pointer"
                                @click="showPassword2 = !showPassword2"/>
                    </template>
                </q-input>

                <q-btn no-caps class="send-btn q-mt-lg full-width" icon="far fa-save" :label="$tl('changeBtn')"
                       @click="changePassword" :loading="submitting">
