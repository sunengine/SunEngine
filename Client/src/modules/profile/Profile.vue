            ...
            <div class="profile__container">
                <div class="profile__img-block flex column">
                    ...
                    <div v-if="messageButtons" class="profile__private-messages-block flex q-mt-sm">
                        ...
                        <q-btn :color="!user.iBannedHim ? 'info' : 'negative'" class="profile__bun-btn shadow-1 q-ml-sm"
                               dense
                               v-if="!user.noBannable" icon="fas fa-ellipsis-v">
                            <q-menu>
                                <q-btn no-caps v-close-popup color="negative" v-if="!user.iBannedHim" @click="ban"
                                       icon="fas fa-ban"
                                       :label="$tl('banBtn')"/>
    ...
    export default {
        ...
        computed: {
            canPrivateMessage() {
                ...
                if (!from)
                    return false;
                if (this.user.heBannedMe || this.user.iBannedHim)
                    return false;
