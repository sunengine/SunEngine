...
namespace SunEngine.Admin.Services
{
    public class ImageCleanerAdminService
    {
            ...
            IPathService pathService)
        {
            ...
            uploadDirectory = pathService.MakePath(globalOptions.CurrentValue.ImagesUploadDir);
        ...
        {
            var photos = await dataBaseConnection.Users.Where(x => x.Photo != null).Select(avatar =>
                    avatar.Photo.Replace('/', Path.DirectorySeparatorChar))
                .ToListAsync();
            var avatars = await dataBaseConnection.Users.Where(x => x.Avatar != null).Select(avatar =>
